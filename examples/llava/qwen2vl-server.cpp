...
static struct llava_image_embed * load_image(llava_context * ctx_llava, common_params * params, const std::string & fname) {
    ...
    if (prompt_contains_image(prompt)) {
        LOGPF("[qwen2vl] load_image @prompt_contains_image, prompt: %s", params->prompt.c_str());
        ...
        LOGPF("[qwen2vl] load_image from filename, prompt: %s", params->prompt.c_str());
...
static void process_prompt(struct llava_context * ctx_llava, struct llava_image_embed * image_embed, common_params * params, const std::string & prompt) {
    ...
    LOGPF("[qwen2vl] @process_prompt, user_prompt: %s", user_prompt.c_str());
...
public:
    ...
    {
        ggml_time_init();
        if (!common_params_parse(argc, argv, params_, LLAMA_EXAMPLE_LLAVA, print_usage)) {
            return 1;
        }
    
        common_init();
    
        if (params_.mmproj.empty() || (params_.image.empty() && !prompt_contains_image(params_.prompt))) {
            print_usage(argc, argv);
            return 1;
        }
    
        model_ = llava_init(&params_);
        if (model_ == NULL) {
            fprintf(stderr, "%s: error: failed to init llava model\n", __func__);
            return 1;
        }

        ctx_llava_ = llava_init_context(&params_, model_);
    ...
    {
        /** load models */
        if(LoadModel(argc, argv) != 0)
        {
            LOG_ERR("load model fail.\n");
            return false;
        }

        const char* ip_addr = params_.hostname.c_str();
        const uint16_t ip_port = params_.port;
...
int main(int argc, char ** argv)
{
    ...
    {
        RLOGI("qwen2vl-server main heartbeat: %d", heartbeat++);
        sleep(10);
    }
